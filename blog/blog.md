...
**There is no dedicated eval suite yet.** Testing has been manual: colleagues run the skill cold, and we read the transcript and the report. Furthermore, we have run a dynamically generated eval suite generated by Anthropic's skill-creator plugin. Apart from these facts, there is no dedicated eval suite running on a specific schedule or triggered by specific events. In the future, we need to design and implement robust tests to ensure the tool behaves as expected in a number of scenarios, so that we can grade it accordingly. his would also provide a means for our users to reproduce our testing scenario exactly, while also providing the ability for us to compare how changing the underlying model affects the outputs.
